...
public class Tuohai_DiffCallBack extends DiffUtil.Callback {
    //旧的数据集合
    private List<TuohaiBean> mOldUserList;
    // 新的数据集合
    private List<TuohaiBean> mNewUserList;

    //构造方法 传入旧的数据结构和新的数据结构


    public Tuohai_DiffCallBack(List<TuohaiBean> mOldUserList, List<TuohaiBean> mNewUserList) {
...
// 如果我们的id一样，在系统里面我就认为两个数据记录是一样的
    ...
    public boolean areItemsTheSame(int oldPosition, int newPosition) {
        return mOldUserList.get(oldPosition).getId()==mNewUserList.get(newPosition).getId();
    }
...
// 这里可以填写自己的逻辑，如果图像是一致的，我就认为内容没有变化
    ...
    public boolean areContentsTheSame(int oldPosition, int newPosition) {
       return mOldUserList.get(oldPosition).getContent().equals(mNewUserList.get(newPosition).getContent());
    }
