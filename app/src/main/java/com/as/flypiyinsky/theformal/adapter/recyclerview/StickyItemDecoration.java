...
public class StickyItemDecoration extends RecyclerView.ItemDecoration {
     ...
     */
    private LengFengAdapter mAdapter;
    /**
     * 标记：UI滚动过程中是否找到标题
     */
     ...
     * 标题的视图View
     */
    private RelativeLayout mStickyItemView;
    /**
     * 承载子项视图的holder
     */
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0, size = parent.getChildCount(); i < size; i++) {//viewgroup.getChildCount()：获取所有可见子元素个数。
            ...
            if ((boolean) item.getTag()) {//判断第几个子项是标题（值在Adapter中设置）
                    ...
                    if (mStickyPositionList.size() > 0) {
                        if (mStickyPositionList.size() == 1) {//若只缓存一个标题
                            ...
                            if (indexOfCurrentPosition > 0) {
                                bindDataForStickyView(mStickyPositionList.get((indexOfCurrentPosition - 1) % mStickyPositionList.size()), parent.getMeasuredWidth());
                            }
                        }
                    }
                }
    ...
    private void getStickyViewHolder(RecyclerView recyclerView) {
        ...
        mAdapter = (LengFengAdapter) recyclerView.getAdapter();
        mViewHolder = mAdapter.onCreateDefViewHolder(recyclerView, LengfengBean.Type0); //该方法属于Adapter中的重写Override

        mStickyItemView = (RelativeLayout) mViewHolder.itemView;//得到布局


        int colorPrimaryDark = R.color.colorPrimaryDark;
        int colorPrimary = R.color.colorPrimary;
        int colorAccent = R.color.colorAccent;
        int[] colors={colorPrimaryDark,colorPrimary,colorAccent};
        Random random=new Random();
        int i = random.nextInt(3);
        DevShape.getInstance(DevShape.RECTANGLE)
                .line(2, colors[i])
                .radius(8)
                .solid(R.color.selector_itemIcon_gray_color)
                .into(mStickyItemView);

        TextView textView=new TextView(mStickyItemView.getContext());
        textView.setText("俺永远在这等你");
        ...
        mStickyItemView.addView(textView);
    ...
    private void bindDataForStickyView(int position, int width) {
        ...
        mAdapter.onBindViewHolder((BaseViewHolder) mViewHolder, mBindDataPosition);//改变标题的展示效果，该方法在Adapter中
